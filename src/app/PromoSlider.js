        ...
        {slides.map((slide, index) => (
          <SwiperSlide key={index}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
              <Image src={slide.pic} alt={slide.title} style={{ width: '100%', height: 'auto', maxHeight: '300px', objectFit: 'cover' }} />
              <h2>{slide.title}</h2>
              <p>{slide.description}</p>
              <small>{slide.uptodate}</small>
              <a href={slide.link} style={{ marginTop: '10px', textDecoration: 'underline', color: 'blue' }}>Узнать больше</a>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
